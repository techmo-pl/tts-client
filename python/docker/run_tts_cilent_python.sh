...
# Requires "tts-client-python:2.0.0" docker image loaded locally
...
docker_image="tts-client-python:2.0.0"
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        -t|--text)                   text="$2"; shift ;;                                    
        -i|--input-text-file)        input_text_file="$2"; shift ;;
...
if [[ "${text}" = "UNSPECIFIED" && "${input_text_file}" = "UNSPECIFIED" ]]; then
    echo "No input specified! Use either '-t | --text TEXT' to specify text to be synthesized or '-i | --input-text-file INPUTFILE' to use text-file input."
    exit 0
elif [[ "${text}" = "UNSPECIFIED" ]]; then
    input_text_file_option="--input_text_file /volumen/txt/${input_text_file##*/}"
    text=""
else
    text_option="--text"
fi
...
python3 /tts_client/tts_client.py \
${service_address_option} \
${session_id_option} \
${grpc_timeout_option} \
${list_voices_option} \
